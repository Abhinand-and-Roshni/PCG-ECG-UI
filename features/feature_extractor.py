...
class Features:
    ...
    def extract_features(self, filter_bandwidth, n_signals=None, show=False, labels=None,
        ...
        print("print 51: ", file_names)
        print("CHECKKKKKK THE VERSION!!!!!!!!!!!", pd.__version__)
    ...
    def _preprocess_signal(self, signal_raw, filter_bandwidth, normalize, polarity_check,
        ...
        ecg_object = ecg.ecg(
            signal=signal_raw, sampling_rate=self.fs, show=False)
        ...
        templates, rpeaks = self._extract_templates(
            signal_filtered, rpeaks, template_before, template_after)
        templates_ts = np.linspace(-template_before,
                                   template_after, templates.shape[1], endpoint=False)
    ...
    def _group_features(self, file_name, ts, signal_raw, signal_filtered, rpeaks,
        ...
        for feature_group in self.feature_groups:
            ...
            if feature_group == 'full_waveform_features':
                ...
                features.update(
                    full_waveform_features.get_full_waveform_features())
                f123 = features

            elif feature_group == 'rri_features':
                rri_features = RRIFeatures(ts=ts, signal_raw=signal_raw,
                                           signal_filtered=signal_filtered, rpeaks=rpeaks,
                                           templates_ts=templates_ts, templates=templates, template_before=template_before,
                                           template_after=template_after,
                                           fs=self.fs)
                rri_features.calculate_heart_rate_variability_statistics()
                features.update(rri_features.get_rri_features())

                #new_fname = "a0001"
                # features.update(file_name=new_fname)
                f123 = features
                print("IS THIS PRINTING???? TEST 1")
                print("261:", features)
                print("IS THIS PRINTING???? TEST 3")
                df4 = pd.DataFrame(features, columns=['t'])
                print("IS THIS PRINTING???? TEST 2")
                print("DF:", df4)
